...
両候補のdecode・正規化時間に実用上の差はなく、通常表示の色味にも差はなかった。最大4倍ではどちらもぼけるが、4096px・12MP候補がわずかに良好だった。細線・文字・斜線・大小の円を含む2400×1600の固定fixtureもSafari・Chromeの両方で確認し、4096px・12MP候補と2160px候補はいずれも最大4倍で十分に鮮明だった。fixtureは4096px・12MP候補では拡大・縮小されず、2160px候補では2160×1440へ縮小されるため、この比較は元寸法と2160px縮小後の細部品質を対象とする。広色域・HDRの色保持に関する厳密な確認は実施していないが、初期リリースで要求する画質に対しては通常の実機写真と固定fixtureによる比較で十分と判断し、完了条件にはしない。
...
- 細線・文字・斜線の固定fixtureはSafari・Chromeの両方で確認済みで、両候補とも最大4倍で十分に鮮明だった。広色域・HDRの色保持に関する厳密な確認は行っていないが、通常の実機写真では許容できない色差を認識せず、初期リリースの完了条件にはしない。
