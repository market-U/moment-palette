...
- 権限拒否、カメラ切替、バックグラウン移行、画面離脱で、意図しない再取得やtrackの解放漏れを防げるか。
...
| デスクトップブラウザ | 基本操作とカメラの事前確認 | 未実施 |
| iPhone 15 / iOS 26 / Safari | 主要な実機判定 | PRプレビュ作成後に実施 |
| iPhone 15 / iOS 26 / Chrome | Safariとの差分確認 | PRプレビュ作成後に実施 |
...
- 拒否、キャンセル、バックグラウン移行、route離脱の後は、明示操作までカメラを再取得しない。
...
初回確認で、表示比率の中間値で選択中エリアも薄くなることと、シャッターとプレビュが離れて同時に見づらいことを確認した。source planeを常に不透明で描いてartwork planeだけを重ねる合成へ変更し、カメラ表示中のシャッターをviewport下部へ固定した。修正後の結果は再確認後に追記する。
...
- 実機でのmask境界のhalo、色、crop、カメラ切替、連続操作時の追従性は未判定である。
