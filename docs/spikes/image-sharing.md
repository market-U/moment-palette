...
- [x] PNGが`image/png`、1080×1080で表示される。
- [x] 画像を長押しして写真へ保存でき、見た目と寸法が一致する。
- [x] 基準`text/plain`方式で共有シートが開き、画像として認識される。
- [x] 標準`image/png`方式で共有シートが開き、基準方式との差を確認できる。
- [x] 画像のみ方式で、同時共有項目による差を切り分けられる。
- [x] 共有シートから写真へ保存でき、見た目と寸法が一致する。
- [x] 共有先の下書きで画像、固定文、ハッシュタグ、URLの受信有無を確認できる。
- [x] 共有シートを閉じてもエラー表示にならず、同じ画面へ戻る。
- [x] 共有文をClipboardへコピーでき、失敗時も手動選択できる。
- [x] 同じPNGを複数回共有してもgeneration idと生成回数が変わらない。
- [x] 再生成後は版番号が変わり、古い画像が表示・共有されない。
- [x] 10回以上の共有・キャンセル・再共有で操作不能や継続的な劣化がない。
...
iPhone 15（iOS 26）のSafariとChromeでPRプレビューを確認した。初回生成はいずれもgeneration id 1、生成回数1で、Safariは102.0 ms、Chromeは75.0 msだった。三経路の`canShare({ files })`は両ブラウザですべて`true`だった。

長押しと共有シートのどちらからも1080×1080 PNGを写真へ保存できた。基準方式と標準方式は、X、LINE、Bufferへ画像、固定文、ハッシュタグ、URLを渡せた。BlueskyはブラウザやFile typeによらず画像だけを受け取ったため、共有先側の対応差として扱う。画像のみ方式は同じ共有先へ画像だけを渡し、文、ハッシュタグ、URLを含めないことを確認した。

共有キャンセルはエラーにならず、共有文のコピーも成功した。同じPNGの再共有でgeneration idと生成回数は増えず、再生成後は新しいVERSIONだけが表示・共有された。SafariとChrome、三経路をまたいで10回以上操作しても、操作不能、古い画像の再表示、継続的な劣化は発生しなかった。

## 経路・ブラウザ別結果

| ブラウザ | 経路 | File type | `canShare()` | 画像認識 | 固定文 | ハッシュタグ | URL | 結果・制約 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Safari | 基準 | `text/plain` | `true` | 成功 | 成功 | 成功 | 成功 | X・LINE・Bufferで全項目を確認 |
| Safari | 標準 | `image/png` | `true` | 成功 | 成功 | 成功 | 成功 | 基準方式との差なし |
| Safari | 画像のみ | `image/png` | `true` | 成功 | 対象外 | 対象外 | 対象外 | 画像だけを引き渡した |
| Chrome | 基準 | `text/plain` | `true` | 成功 | 成功 | 成功 | 成功 | Safariとの差なし |
| Chrome | 標準 | `image/png` | `true` | 成功 | 成功 | 成功 | 成功 | 基準方式との差なし |
| Chrome | 画像のみ | `image/png` | `true` | 成功 | 対象外 | 対象外 | 対象外 | 画像だけを引き渡した |

## 共有先別結果

| ブラウザ | 経路 | 共有先 | 画像 | 文・タグ・URL | 投稿・送信前に確認 | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| Safari | 基準・標準 | X・LINE・Buffer | 成功 | 成功 | 成功 | File typeによる差なし |
| Safari | 基準・標準 | Bluesky | 成功 | 未受信 | 成功 | Bluesky側が画像とテキストの同時受信に未対応 |
| Safari | 画像のみ | X・LINE・Buffer・Bluesky | 成功 | 対象外 | 成功 | 画像だけを受信 |
| Chrome | 基準・標準 | X・LINE・Buffer | 成功 | 成功 | 成功 | Safariとの差なし |
| Chrome | 基準・標準 | Bluesky | 成功 | 未受信 | 成功 | Bluesky側が画像とテキストの同時受信に未対応 |
| Chrome | 画像のみ | X・LINE・Buffer・Bluesky | 成功 | 対象外 | 成功 | 画像だけを受信 |

## 保存・resource結果

| ブラウザ | 長押し保存 | 共有シート保存 | 保存寸法 | Blob再利用 | 再生成 | 10回操作 |
| --- | --- | --- | --- | --- | --- | --- |
| Safari | 成功 | 成功 | 1080×1080 PNG | 成功 | 新VERSIONだけを表示・共有 | Chromeと合わせて10回以上、劣化なし |
| Chrome | 成功 | 成功 | 1080×1080 PNG | 成功 | 新VERSIONだけを表示・共有 | Safariと合わせて10回以上、劣化なし |

## 採否

- 基準の`text/plain`方式を初期の製品実装へ採用する。iPhone Safari・Chromeで標準方式と同じ受信結果になり、Androidを含む既存アプリで3年以上運用中という根拠も維持できるためである。
- 標準の`image/png`方式はiPhoneでは問題なく成立したが、既知のAndroid課題を今回再確認できていないため主経路には採用しない。将来Android実機で比較する候補として設計判断を残す。
- 画像のみ方式は切り分けに有効だったが、固定文を添える製品要件を満たさないため製品UIには採用しない。
- 通常の`img`による長押し保存と、OS共有シートからの保存を採用する。独自ダウンロードボタンは追加しない。
- 共有文のClipboardコピーと手動選択可能な表示を、File共有非対応・共有先差へのfallbackとして採用する。
- 一つのPNG Blobを表示と再共有へ再利用し、再生成・画面離脱時にobject URLを破棄する所有権方式を採用する。
- Blueskyが文、ハッシュタグ、URLを受け取らない挙動は共有先側の制約であり、アプリの失敗とは扱わない。

## コードの扱い

- `shareOutcome.ts`、`sharePort.ts`、`clipboardPort.ts`、`completedImagePort.ts`、`completedImageOwner.ts`と対応する単体テストは、本実装featureへ移動・改名した上で昇格する候補とする。
- `browserClipboard.ts`と対応テストは、本実装のClipboard adapterへ昇格する候補とする。
- `sharePayload.ts`と`browserImageShare.ts`は、基準方式だけを製品経路にし、最終の共有文・ハッシュタグ・URLと命名を確定してから設計を保って再実装する。
- `canvasCompletedImageGenerator.ts`はF/S fixtureであり、本実装では制作機能のcompositorを使用するため削除する。
- F/S専用route、page、比較UI、診断表示、固定文、Canvas fixtureとそれらのF/Sテストは、本実装changeで必要なmoduleを移した後に削除する。

## 後続文書と本実装への反映

- `docs/vision.md`の製品要求は、1080×1080 PNG、長押し保存、Web Share、Clipboard fallback、Blob再利用をすでに含み、今回の結果で変更されない。`text/plain`の採用は実装方式なので、このF/Sからvisionへ新しい製品要求は追加しない。
- 画面構成と操作要求は変わらないため、`docs/design/`は更新しない。製品UIの最終調整は完成確認・共有画面の本実装changeで行う。
- `docs/development-roadmap.md`では、このF/Sを実装・実機確認完了として記録し、次の中核技術F/SをAzureテンプレート配信とする。
- 後続の完成確認・共有画面の本実装changeへ、基準方式、resource所有権、キャンセル分類、長押し保存、Clipboard fallback、共有先差を正式なspecとdesignとして記述する。

## 制約と未解決事項

- Androidでは既存アプリの継続運用実績があるが、Moment Palette固有の新規実機確認は行わない。
- 共有先はShareDataの一部を破棄または統合できるため、Webアプリは各項目の受信を保証できない。
- BlueskyはSafari・Chrome、基準・標準のいずれでも画像だけを受け取り、共有文を受け取らなかった。
