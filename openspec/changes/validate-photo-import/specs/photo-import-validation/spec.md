...
検証実装は、正規化済み写真を1080×1080作品領域へcoverして表示し、表示比率、1本指pan、2本指pinch、最大4倍の倍率制限、余白を生じさせない移動制限を提供しなければならない（SHALL）。選択中の`文鳥01`mask内へ写真を描き、線画を最前面へ表示しなければならない（MUST）。
...
- **THEN** pointerが領域外へ移動しても操作が継続し、制限後の位置で作品領域に写真のない余白を表示しない

#### Scenario: 写真をpinchする
- **WHEN** 検証者が編集領域内の2本指間隔を変更する
- **THEN** 写真は指の中点を基準にcover倍率から最大4倍まで連続的に拡大縮小される
