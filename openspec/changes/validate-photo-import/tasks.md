...
- [x] 5.10 確定済みレイヤーを選択中エリアより下と上の静的平面へ事前合成し、pointer操作中はレイヤー数に依存せず固定三段だけを描くキャッシュと単体テストを追加する
...
- [x] 6.10 pan・pinchのgesture計算は各pointer eventで更新しつつ、Canvas描画要求を`requestAnimationFrame`で一画面更新につき一回へ集約し、破棄時に待機中の描画を取り消す
...
- [x] 7.6 静的レイヤーキャッシュと描画要求集約の単体テストを含めて全品質検査を再実行し、既存の写真・カメラF/Sに回帰がないことを確認する
...
- [x] 9.1 iPhone 15（iOS 26）のSafariで、写真ライブラリ、提示される場合のOSカメラ、ファイル選択を開き、同じ写真処理へ合流することと選択キャンセルを確認する
- [x] 9.2 SafariでHEIC・HEIF由来写真、JPEG、alpha付きPNGを各入口から選び、返却された拡張子・MIME type、decode経路、成否、ブラウザによる形式変換の有無を記録する
- [x] 9.3 SafariでOrientation 1・3・6・8 fixtureと縦横の実機写真を選び、プレビュー、mask合成、確定画像の向きと寸法が一致することを確認する
