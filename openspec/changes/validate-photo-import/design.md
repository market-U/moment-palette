...
作品座標は1080×1080とし、正規化済み写真を正方形全体へcoverする初期変換を作る。画像形式による配置モードは分けず、すべての画像を選択エリアの外接矩形へ収まる倍率の4分の1まで縮小できるようにする。offsetは画像の一部が選択エリア内へ残る範囲に制限し、余白にはそのエリアの初期色を表示する。これにより通常写真は従来の初期表示を保ちつつ、透過PNGや小さな素材も同じ操作で配置できる。編集canvasだけへ`touch-action: none`を設定し、Pointer Eventsとpointer captureでpan・pinchを処理する。

描画は`source plane`へ写真全体、`artwork plane`へ現在の作品と選択中mask内の写真を描き、表示比率で重ねた後、線画を最前面へ一度だけ描く。選択中エリアの写真は中間値でも薄くしない。各エリアでは初期色を描いてから写真を重ねるため、作品表示100%では透過PNGの透明画素や画像外の余白から、重なっている別エリアの写真を露出させない。0〜99%では比較用`source plane`として選択写真全体を意図的に表示する。写真確定時は変換込みの1080×1080 detached canvasを選択エリアの状態として保持できるところまで確認するが、完成PNGの保存・共有は別changeとする。

pointermoveごとに同期描画すると、同じ画面更新内で古い途中状態まで描いてしまうため、pan・pinchによる描画要求は`requestAnimationFrame`で一画面更新につき一回へ集約する。gesture計算と最新transformの更新は各pointer eventで続け、描画時には常にその時点の最新transformを使う。

確定済みレイヤーは、選択中エリアより下の静的平面と上の静的平面へ事前合成する。操作中は「下の静的平面、選択中エリア、上の静的平面」の固定三段だけを描き、確定済みレイヤーを走査・mask合成し直さない。静的平面の再構築は、写真の調整開始、確定、キャンセル、エリア切替など作品構成が変わる時点に限る。単一の静的平面へ選択中エリアを最後に重ねる方式は、エリア同士の前後関係を変えて透過部分の見え方を壊すため採用しない。
...
- [レイヤー数の増加によりpan・pinch中のmask合成回数が増える] → 確定済みレイヤーを選択中エリアの前後へ分けてキャッシュし、操作中の描画量をレイヤー数から独立させる。キャッシュ用Canvasの追加メモリとの釣り合いは実機操作で確認する。
