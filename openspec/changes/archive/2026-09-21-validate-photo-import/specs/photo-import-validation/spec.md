...
#### Scenario: 連続するpointer操作を描画する
- **WHEN** panまたはpinchによる複数のpointer更新が一回の画面更新より短い間隔で発生する
- **THEN** 最新の変換を保持したままCanvas描画は一画面更新につき一回へ集約される
- **AND** pointer操作中は確定済みレイヤーを再合成せず、選択中エリアより下と上の静的キャッシュを再利用する
