...
初期本番のAzure Static Web Apps基盤、HTTPS配信、SPA設定、release配信、PRプレビュー、および実機確認を定める。
...
初期本番SWAは、iPhone Safari、iPhone Chrome、およびPCブラウザからHTTPSでアクセスでき、ProductionとPR previewの両方でアプリケーションシェルを表示できなければならない（SHALL）。
...
#### Scenario: iPhoneからProduction環境を確認する

- **WHEN** 検証者がiPhone SafariおよびiPhone ChromeでProductionの固定HTTPS URLを開く
- **THEN** 各ブラウザは証明書エラーなしでアプリケーションシェルを表示する

#### Scenario: PCから作品を完成する

- **WHEN** 検証者がPCブラウザでProductionまたはPR previewの作品を完成する
- **THEN** アプリケーションは保存および共有操作を提供する完成画面を表示する
